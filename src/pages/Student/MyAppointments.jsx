...
const MyAppointments = ({ student, teacher }) => {
  ...
  const getMyAppointmentsList = async () => {
    try {
            ...
            <tbody>
              {myAppointmentList.map((appointment) => (
                <tr>
                  ...
                  <td>
                    {
                      appointment.status === 'approved' ? (<span style={{color:'green'}}>Approved</span>)
                      :
                      appointment.status === 'declined' ? (<span style={{color:'red'}}>Cancelled</span>) 
                      :
